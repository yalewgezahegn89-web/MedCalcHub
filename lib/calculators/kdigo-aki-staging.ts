...
export const kdigoAkiStagingCalculator: CalculatorDefinition = {
  ...
  clinicalNotes:
    "The KDIGO (Kidney Disease: Improving Global Outcomes) 2012 framework classifies acute kidney injury (AKI) severity into stages 1–3 using two independent sets of criteria: serum creatinine (an absolute rise of ≥0.3 mg/dL within 48 hours, or a rise of 1.5–1.9×, 2.0–2.9×, or ≥3.0× baseline within 7 days) and urine output (oliguria thresholds expressed per mL/kg/h over specified durations). Staging classifies severity and risk; it does not, by itself, establish the cause of AKI.\n\nThe staging result depends on which baseline creatinine is used, and the true baseline is often unknown at presentation, particularly when the current value is already elevated. An estimated baseline can be substituted (e.g., the lowest creatinine in the preceding 7 days), but this introduces uncertainty that can shift the assigned stage; the same creatinine pair can produce a different stage depending on the baseline chosen.\n\nUrine output criteria must be interpreted with caution. They reflect recent volume status, the presence of diuretics, bladder catheter patency, and the accuracy of the recorded output as much as intrinsic kidney function, and brief or ambiguous low-output readings can misclassify the stage. Staging identifies whether AKI is present and its severity; it does not identify AKI etiology (whether the injury is prerenal, intrinsic, or postrenal), and it does not itself prescribe a specific treatment.\n\nThe KDIGO framework is intended primarily for adults; pediatric and pregnancy-specific frameworks use separate definitions, and some special populations require alternative criteria. Trends over time matter as much as the single calculated stage: a changing creatinine trajectory and the clinical context should guide interpretation rather than the number alone.",
